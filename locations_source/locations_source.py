...
class LocationsSource(object):
    ...
    
    {
        id: {
            ...
            country_id: int,
            admin1: Optional[str],
            admin1_id: Optional[int],   # can be 0 if admin1 is not present in data set
            admin2: Optional[str],
            admin2_id: Optional[int],   # can be 0 if admin2 is not present in data set
            city: Optional[str],
    ...
    def city_search(self, city_name):
        city_name = re.sub(r'^St\b', 'St.', city_name, flags=re.IGNORECASE)
        city_name = re.sub(r'^N\.?Y\.?C\.?$', 'New York City', city_name, flags=re.IGNORECASE)
        city_name = re.sub(r'Manhattan', 'New York City', city_name, flags=re.IGNORECASE)
        city_name = re.sub(r'^City\s+Of\s+', '', city_name, flags=re.IGNORECASE)
        city_name = standardize_loc_name(city_name)

        return dict(
            (id_, loc) for id_, loc in self._locations_by_name[city_name].iteritems()
            if loc['resolution'] == ResolutionTypes.CITY
        )

    def admin1_search(self, admin1_name):
        admin1_name = standardize_loc_name(admin1_name)
        return dict(
            (id_, loc) for id_, loc in self._locations_by_name[admin1_name].iteritems()
            if loc['resolution'] == ResolutionTypes.ADMIN_1
        )

    def _admin2_search(self, admin2_name):
        admin2_name = standardize_loc_name(admin2_name)
        return dict(
            (id_, loc) for id_, loc in self._locations_by_name[admin2_name].iteritems()
            if loc['resolution'] == ResolutionTypes.ADMIN_2
        )

    def admin2_search(self, admin2_name):
        locations = self._admin2_search(admin2_name)
        locations.update(self._admin2_search('%s County' % admin2_name))
        return locations
    ...
    def country_search(self, country_name):
        ...
        return dict(
            (id_, loc) for id_, loc in self._locations_by_name[country_name].iteritems()
    ...
    def all_locations_search(self, name):
        matches = {}
        for lookup in (
            self.city_search, self.admin1_search, self.admin2_search, self.country_search,
        ):
            matches.update(lookup(name))
        return matches

    def get_location_by_id(self, id_):
        return self._locations_by_id[id_]
